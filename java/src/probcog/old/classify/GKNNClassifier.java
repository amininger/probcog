 ...
 *  of the neighbors
 */
public class GKNNClassifier implements Classifier
{
    ...
    double p;  // Covariance parameter
    String filename = null;    // Load data from this file
     ...
     */
    public GKNNClassifier(int k_, double p_)
    {
        k = k_;
        p = p_;
    }
    ...
    synchronized public void add(ArrayList<Double> features, String label)
    {
        if (label == null)
            return;
        double[] dfeatures = Util.toArray(features, new double[features.size()]);
        add(dfeatures, label);
    }
    ...
    {
        return classify(Util.toArray(features, new double[features.size()]));
    }
    ...
    {
        // Test code!
        GKNNClassifier knn = new GKNNClassifier(10, 0.1);
    ...
    static private void test(GKNNClassifier knn)
    {
        // Test colors
