...
from mobilesim.mapmaker.parse_info_file import parse_info_file

from mobilesim.mapmaker.create_world import create_world
from mobilesim.mapmaker.create_waypoint_map import create_waypoint_map
from mobilesim.mapmaker.create_map_info import create_map_info
from mobilesim.mapmaker.create_internal_world import create_internal_world

#################### CHECK ARGUMENTS ##############################

if len(sys.argv) <= 1:
	print("python3 -m mobilesim.mapmaker [map_info_file]")
	print("  map_file_file is a specification of the world to create in $MOBILE_SIM_HOME/map_info")
	print("Will generate a set of world/map files for a rosie environment")
	print("")
	print("Example: python3 -m mobilesim.mapmaker simple_office.info")
	print("  Input File : $MOBILE_SIM_HOME/map_info/simple_office.info")
	print("")
...
# Lookup $MOBILE_SIM_HOME environment variable [REQUIRED]
mobile_sim_home = ""
if "MOBILE_SIM_HOME" in os.environ:
	mobile_sim_home = os.environ["MOBILE_SIM_HOME"]
else:
	print("ERROR: Requires MOBILE_SIM_HOME environment variable set")
	sys.exit(0)
